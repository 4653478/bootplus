 ...
 * 日志AOP切入
 *
 * @author Created by age on 2020/1/10
 */
@Aspect
@Component
@Slf4j
...
public class LoggingAspect {
     ...
     */
    @Pointcut("within(com.age.controller.*.*)")
     ...
     */
//    @Pointcut("within(com.age.service.*.*)")
...
//        log.debug("被代理对象是否是jdk动态接口代理调用:{}", AopUtils.isJdkDynamicProxy(joinPoint.getTarget()));
    ...
    /**
     * 指定切点(匹配`com.age.controller`包及其子包下的所有类的所有方法)
     * execution用于匹配方法执行的连接点
     */
    @Pointcut("execution(* com.age.controller.*.*(..))")
    public void optLog() {
    }
     ...
     * 签名表达式
     */
    @Pointcut(value = "@annotation(com.age.config.aop.annotation.MyLog)")
